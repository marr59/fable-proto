...
// ── Юниты: риггованная модель Xbot.glb (скелетная Idle/Walk) ──
const XBOT_URL = 'https://raw.githubusercontent.com/mrdoob/three.js/r168/examples/models/gltf/Xbot.glb';
const CLASS_COLOR = { peasant:0xb58a4a, archer:0x3f7a38, spearman:0x9c3a33, alchemist:0x5c3f92 };
let baseGLTF = null, UNIT_SCALE = 1, FOOT_Y = 0;

function makeWeapon(cls){
 if(cls==='archer'){
 const w=new THREE.Group();
 const bow=new THREE.Mesh(new THREE.TorusGeometry(0.34,0.022,8,20,Math.PI*1.25),new THREE.MeshStandardMaterial({color:0x5a3a18,roughness:0.7,flatShading:true})); bow.rotation.z=Math.PI/2; bow.castShadow=true; w.add(bow);
 w.position.set(0.36,0.95,0.05); return w;
 }
 if(cls==='spearman'){
 const w=new THREE.Group();
 const shaft=new THREE.Mesh(new THREE.CylinderGeometry(0.03,0.03,2.0,8),new THREE.MeshStandardMaterial({color:0x5a3d1e,roughness:0.85,flatShading:true})); shaft.position.y=1.0; shaft.castShadow=true; w.add(shaft);
 const blade=new THREE.Mesh(new THREE.ConeGeometry(0.05,0.34,4),new THREE.MeshStandardMaterial({color:0xc4ccd4,metalness:0.7,roughness:0.25,flatShading:true})); blade.position.y=2.12; w.add(blade);
 const tassel=new THREE.Mesh(new THREE.ConeGeometry(0.055,0.15,8),new THREE.MeshStandardMaterial({color:0x8a1520,roughness:0.85,flatShading:true})); tassel.position.y=1.86; tassel.rotation.x=Math.PI; w.add(tassel);
 w.position.set(0.34,0,0); return w;
 }
 if(cls==='alchemist'){
 const w=new THREE.Group();
 const staff=new THREE.Mesh(new THREE.CylinderGeometry(0.028,0.032,1.9,8),new THREE.MeshStandardMaterial({color:0x3a2a20,roughness:0.85,flatShading:true})); staff.position.y=0.95; staff.castShadow=true; w.add(staff);
 const orb=new THREE.Mesh(new THREE.SphereGeometry(0.1,12,10),new THREE.MeshStandardMaterial({color:0x9a3ad0,emissive:0x9a3ad0,emissiveIntensity:2,flatShading:true})); orb.position.y=1.95; w.add(orb);
 w.position.set(0.34,0,0); return w;
 }
 return null;
}
...
function spawnUnit(cls, x, z){
 ...
 model.scale.setScalar(UNIT_SCALE); model.position.y = FOOT_Y;
 ...
 group.add(model);
 let weapon = makeWeapon(cls); if(weapon) group.add(weapon);
 ...
 idleAction.play();
 const u = { group, model, weapon, ring, cls, mixer, idleAction, walkAction, activeAction:idleAction, target:new THREE.Vector3(x,0,z), moving:false, order:null, train:0, facing:0, bob:Math.random()*6 };
...
new GLTFLoader().load(XBOT_URL, (gltf)=>{
 baseGLTF = gltf;
 const box=new THREE.Box3().setFromObject(gltf.scene);
 const h=Math.max(0.001, box.max.y-box.min.y);
 UNIT_SCALE = 1.7/h; FOOT_Y = -box.min.y*UNIT_SCALE;
...
}, undefined, err=>console.warn('Xbot load error:', err));
...
function transform(u, cls){
 ...
 if(u.weapon){ u.group.remove(u.weapon); u.weapon=null; }
 u.weapon = makeWeapon(cls); if(u.weapon) u.group.add(u.weapon);
